...
const ContactForm = ({ onSubmitCallback }) => {
  ...
  const handleSubmit = (e) => {
    ...
    // su: subject
    // body: body text
    const gmailUrl = `https://mail.google.com/mail/?view=cm&fs=1&to=Bitcoinafricastory@proton.me&su=${encodeURIComponent(formData.subject)}&body=${encodeURIComponent(bodyContent)}`;

    // Open Gmail in a new tab
    window.open(gmailUrl, '_blank');

    setModal({ open: true, title: 'Redirecting to Gmail', message: "A new tab is opening with your Gmail compose window. Please send the email from there!" });
